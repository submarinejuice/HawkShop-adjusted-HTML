...
function getSuitelet(script, deploy) {
   return {"customscript_ns_sc_ext_sl_newsletter_sp:customdeploy_ns_sc_ext_sl_newsletter_sp":"/app/site/hosting/scriptlet.nl?script=642&deploy=1&compid=7763301&ns-at=AAEJ7tMQOZvkGnx_OWIKlUC-BYT-MLGpj3j6j72ARrIj0TzQ0X4","customscript_ns_sc_ext_sl_orderstatus_of:customdeploy_ns_sc_ext_sl_orderstatus_of":"/app/site/hosting/scriptlet.nl?script=644&deploy=1&compid=7763301&ns-at=AAEJ7tMQDuqqksPBu8zlHGvhTG3kw1xmhfNJ_XNOY9mPLaPM0Og","customscript_ns_sc_sl_gift_cert_balance:customdeploy_ns_sc_sl_gift_cert_balance":"/app/site/hosting/scriptlet.nl?script=631&deploy=1&compid=7763301&ns-at=AAEJ7tMQ8Z8yT2Aw8z_j9ytdTdjbSYinjbdJb_vBJhCaHcbbgQc","customscript_ns_sc_sl_gift_cert_groups:customdeploy_ns_sc_sl_gift_cert_groups":"/app/site/hosting/scriptlet.nl?script=630&deploy=1&compid=7763301&ns-at=AAEJ7tMQ5wrI_auz1pH3nyT58mskhLWVWMwr4EykbwCXltzgwG8","customscript_ns_sc_sl_gift_cert_mgmt:customdeploy_ns_sc_sl_gift_cert_mgmt":"/app/site/hosting/scriptlet.nl?script=632&deploy=1&compid=7763301&ns-at=AAEJ7tMQeBflSrXEol0RI9C7HYio5gANP2spLwsYN-7Lg3aRx6k","customscript_ns_sc_sl_itembadges:customdeploy_ns_sc_sl_itembadges":"/app/site/hosting/scriptlet.nl?script=638&deploy=1&compid=7763301&ns-at=AAEJ7tMQTpxny-G9Nh8CAok_crUsqjn2_zTIVb7c_KtVseHRYS8","customscript_nssc_p2go_suitelet:customdeploy_nssc_p2go_suitelet":"/app/site/hosting/scriptlet.nl?script=735&deploy=1&compid=7763301&ns-at=AAEJ7tMQxc62ZXU1Md1e4dTaiih4Ht7fJNNUEoDQA8ToZVabhUs"}[script + ":" + deploy];
}
...
extensions['ACS.SSOLoginLandingPage.1.1.0'] = function(){
function getExtensionAssetsPath(asset){
return 'extensions/ACS/SSOLoginLandingPage/1.1.0/' + asset;
};
define('ACS.SSOLoginLandingPage.View', [
    'acs_ssologin_landingpage_view.tpl',
    'PageType.Base.View',
    'underscore',
    'jQuery'
], function SSOLoginLandingPageView(
    acsSsologinLandingpageTpl,
    PageType,
    _,
    jQuery
) {
    ...
    return PageType.PageTypeBaseView.extend({
        template: acsSsologinLandingpageTpl,
        beforeShowContent: function beforeShowContent() {
            var self = this;
            var promise = jQuery.Deferred();
            this.environment = this.options && this.options.environment;
            this.userProfile = this.options.userProfile;
            this.vitalsourceCartEnabled = this.environment.getConfig('vitalsourceCartEnabled');
            this.vsTokenParam = this.environment.getConfig('vitalsourceCartTokenParameter') || 'vsscatoken';
            this.vsTokenSso = this.getVitalSourceToken();
            this.hasVsLocalToken = this.vitalsourceCartEnabled && !_.isEmpty(this.vsTokenSso);
            this.checkoutTouchPoint = this.environment.getSiteSetting('touchpoints.checkout');
            this.shoppingTouchPoint = this.environment.getSiteSetting('touchpoints.home');
            this.afterLoginUrl = this.environment.getConfig('acsSsoLoginLandingPage.afterLoginUrl');
            this.afterSSOURL = this.afterLoginUrl || this.shoppingTouchPoint;
            this.pageHeading = this.environment.getConfig('acsSsoLoginLandingPage.pageHeading');
            this.userProfile.getUserProfile().done(function done(profile) {
                self.isLoggedIn = profile && profile.isloggedin;
                promise.resolve();
            }).fail(function failed(error) {
                // eslint-disable-next-line no-console
                console.warn('Failed getUserProfile in ACS.SSOLoginLandingPage.View:', error);
                promise.resolve();
            });
            return promise;
        },
        getVitalSourceToken: function getVitalSourceToken() {
            return localStorage.getItem('vsTokenSso');
        },
        removeTokenForSSOPage: function removeTokenForSSOPage() {
            localStorage.removeItem('vsTokenSso');
        },
        getContext: function getContext() {
            var redirectUrl;
            if (this.isLoggedIn) {
                if (this.hasVsLocalToken) {
                    redirectUrl = this.checkoutTouchPoint + '&' + this.vsTokenParam + '=' + this.vsTokenSso;
                    this.removeTokenForSSOPage();
                } else {
                    redirectUrl = this.afterSSOURL;
                }
            }
            return {
                isLoggedIn: this.isLoggedIn,
                pageHeading: this.pageHeading,
                afterLoginUrl: this.afterLoginUrl,
                redirectUrl: redirectUrl
            };
        }
    });
});
define('ACS.SSOLoginLandingPage', [
    'ACS.SSOLoginLandingPage.View',
    'Utils'
], function ACSSSOLoginLandingPage(
    ACSSSOLoginLandingPageView,
    Utils
) {
    'use strict';
    return {
        mountToApp: function mountToApp(container) {
            ...
            var environment = container.getComponent('Environment');
            var userProfile = container.getComponent('UserProfile');
            if (pageType && environment && userProfile) {
                try {
                    pageType.registerPageType({
                        'name': 'ACS.SSOLoginLandingPage.View',
                        'routes': ['sso-login-landing'],
                        options: {
                            environment: environment,
                            userProfile: userProfile,
                            pageInfo: {
                                name: 'SSO Login Landing',
                                title: Utils.translate('SSO Login Landing'),
                                url: 'sso-login-landing'
                            }
                        },
                        'defaultTemplate': {
                            'name': 'acs_ssologin_landingpage_view.tpl',
                            'displayName': 'SSO Login Landing'
                        },
                        'view': ACSSSOLoginLandingPageView
                    });
                } catch (error) {
                    // eslint-disable-next-line no-console
                    console.error('Error registering page type for ACS.SSOLoginLandingPage :' + error);
                }
            }
        }
    };
...
try{
	extensions['ACS.SSOLoginLandingPage.1.1.0']();
	SC.addExtensionModule('ACS.SSOLoginLandingPage');
}
catch(error)
{
	console.error(error)
